...
type Language = {
  key: string;
  title: string;
  flag: StaticImageData; // Используем StaticImageData для импортированных изображений
  altTitle: string;
};
...
const ChangeLanguage: React.FC = () => {
  ...
  useEffect(() => {
    // Получаем сохранённый язык из localStorage при монтировании компонента
    const savedLanguage = localStorage.getItem("currentLanguage") || "ru"; // Значение по умолчанию "ru"
    const language = languages.find(lang => lang.key === savedLanguage) || languages[0]; // Устанавливаем язык по умолчанию
  ...
  const selectLanguage = (key: string) => {
    ...
    if (selectedLanguage) {
      setCurrentLanguage(selectedLanguage); // Устанавливаем выбранный язык
      localStorage.setItem("currentLanguage", key); // Сохраняем выбранный язык в localStorage
    }
    setOpen(false);
  };
